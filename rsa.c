...
int rsa_public_encrypt(uint8_t *out, uint32_t *out_len, uint8_t *in, uint32_t in_len, rsa_pk_t *pk)
{
    ...
    memcpyX((uint8_t *)&pkcs_block[i], (uint8_t *)in, in_len);
    status = public_block_operation(out, out_len, pkcs_block, modulus_len, pk);

    // Clear potentially sensitive information
    byte = 0;
    memsetX((uint8_t *)pkcs_block, 0, sizeof(pkcs_block));
...
int rsa_public_decrypt(uint8_t *out, uint32_t *out_len, uint8_t *in, uint32_t in_len, rsa_pk_t *pk)
{
    ...
    *out_len = modulus_len - i;
    ...
    memcpyX((uint8_t *)out, (uint8_t *)&pkcs_block[i], *out_len);
...
static int public_block_operation(uint8_t *out, uint32_t *out_len, uint8_t *in, uint32_t in_len, rsa_pk_t *pk)
{
    ...
    *out_len = (pk->bits + 7) / 8;
    ...
    // Clear potentially sensitive information
    memsetX((uint8_t *)c, 0, sizeof(c));
    memsetX((uint8_t *)m, 0, sizeof(m));
...
static int private_block_operation(uint8_t *out, uint32_t *out_len, uint8_t *in, uint32_t in_len, rsa_sk_t *sk)
{
    ...
    *out_len = (sk->bits + 7) / 8;
    ...
    // Clear potentially sensitive information
    memsetX((uint8_t *)c, 0, sizeof(c));
    memsetX((uint8_t *)cp, 0, sizeof(cp));
    memsetX((uint8_t *)cq, 0, sizeof(cq));
    memsetX((uint8_t *)dp, 0, sizeof(dp));
    memsetX((uint8_t *)dq, 0, sizeof(dq));
    memsetX((uint8_t *)mp, 0, sizeof(mp));
    memsetX((uint8_t *)mq, 0, sizeof(mq));
    memsetX((uint8_t *)p, 0, sizeof(p));
    memsetX((uint8_t *)q, 0, sizeof(q));
    memsetX((uint8_t *)q_inv, 0, sizeof(q_inv));
    memsetX((uint8_t *)t, 0, sizeof(t));
